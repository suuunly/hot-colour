...
namespace HotColour.Data.Game
{
    public class GameSession
    {
        ...
        public bool AddPlayer(NewPlayer newPlayer, out string playerId)
        {
            ...
            if (_players.TryAdd(playerId, new Player(playerId, name, avatar)))
            {
                _playerOrder = GetPlayers().Select(p => p.Id).ToList();    
            }
        ...
        private void NextPlayer()
        {
            ...
            if (_currentPlayerTurnIndex > _playerOrder.Count - 1)
            {
                _currentPlayerTurnIndex = 0;
            }
        }
        
        public bool StartGame(Func<Task> onRoundEnded)
        {
            if (_players.Count < 2)
            {
                return false;
            ...
            Task.Delay(TimeSpan.FromSeconds(totalTime)).ContinueWith(_ =>
            {
                if(!_players.TryGetValue(PlayerIdOfCurrentPlayer, out Player player))
                {
                    return;
                }
